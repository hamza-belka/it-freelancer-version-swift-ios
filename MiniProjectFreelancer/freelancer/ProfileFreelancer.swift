...
//
//  Created by ESPRIT on 07/12/2019.
...
class ProfileFreelancer: UIViewController ,UITableViewDelegate,UITableViewDataSource{
    ...
    @IBOutlet weak var tofpro: UIImageView!
    let defaultValues = UserDefaults.standard
    override func viewDidLoad() {
        super.viewDidLoad()
        tofpro.layer.cornerRadius = tofpro.frame.size.width/2
        tofpro.clipsToBounds = true
        getprofileinfos()
        getexperience()
        geteducations()
        getskills()
        ...
        Alamofire.request(URL_PROFILEINFOS, method: .post, parameters: parameters).responseJSON
            {
                ...
                if let result = response.result.value {
                    ...
                    self.freelancername.text=self.defaultValues.string(forKey: "full_name")
